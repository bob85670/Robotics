...
class RobotRRT(RobotBase):
	...
	def rrt_nearest(self, x_target):
		...
		for i, node in enumerate(self.nodes):
			...
			dist = np.linalg.norm(x_target - node[0])
			if dist < min_dist:
				min_dist = dist
				x_nearest = node[0]
				index_nearest = i
	...
	def rrt_stear(self, x_nearest, x_rand):
		...
		x_new = x_nearest + expand_dis * direction / distance
	...
	def rrt_near(self, x_target):
		...
		"*** YOUR CODE STARTS HERE ***"
		# Calculate r 
		r = min(gamma * ( (math.log(len(self.nodes))) / (len(self.nodes)) ) ** (1 / d), eta)

		# Loop within tree to find all satisfiable node
		for i, node in enumerate(self.nodes):
			if np.linalg.norm(node[0] - x_target) <= r:
		...
		for i in range(self.p_step_per_iter):
			...
			if self.rrt_collision_free(x_nearest, x_new):
				if self.p_mode == 'rrt':
					...
					near_indexes = [node[1] for node in near_nodes]


					# Line 9-12, loop from the list, find node with
					# the minimum path cost, get its index as well
					min_index = index_nearest
					min_cost_node = self.nodes[index_nearest]
					min_cost = min_cost_node[2] + self.rrt_cost(min_cost_node[0], x_new)
					for node in near_nodes:
						if self.rrt_collision_free(node[0][0], x_new):
							cost = node[0][2] + self.rrt_cost(node[0][0], x_new)
							if cost < min_cost:
								min_cost = cost
					...
					self.nodes.append([x_new, min_index, min_cost])
					...
					for index in near_indexes:
						...
						if index != min_index and self.rrt_collision_free(x_new, node[0]):
							cost = min_cost + self.rrt_cost(x_new, node[0])
							if cost < node[2]:
								node[1] = len(self.nodes) - 1
								node[2] = cost
