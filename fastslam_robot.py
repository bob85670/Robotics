...
class RobotFastSLAM(RobotBase):
	...
	def __init__(self, id, state, vel = np.zeros((2, 1)), goal=np.zeros((3, 1)), 
		...
		self.e_R     = kwargs.get('e_R', np.diag([0.04, 0.01, 0.01, 0.04])) # Noise amplitude of ekf estimation motion model
	...
	@staticmethod
	def motion_model(current_state, vel, step_time, noise_alpha = [0.03, 0, 0, 0.03], seed = np.random.default_rng()):
	...
	def fastslam_particles_motion_predict(self, vel, **kwargs):
		...
		@param particle: a class with attributes: weight, state, landmarks.
						 ...
						 particle.landmarks[0]['std']: 2*2 matrix.
	...
	def fastslam_particles_measurement_update(self, **kwargs):
		...
		@param dt:  delta time
		@param Q_t: 2*2 matrix, the assumed noise amplitude for measurement, usually diagnal.
		@param self.particles: a list containing all the particles, see below.
		@param particle: a class with attributes: weight, state, landmarks.
						 Access by (example):
						 particle.weight: float.
		...
		for particle in self.particles:
			...
			for lm in lm_measurements:
				...
				if not particle.landmarks[lm_id]['observed']:
					...
					particle.landmarks[lm_id]['mean'] = np.array([ particle.state[0] + lm['range']*math.cos(lm['angle']+particle.state[2]), 
																   particle.state[1] + lm['range']*math.sin(lm['angle']+particle.state[2]) ])


					# Calculate Jacobian related to landmark, at line 8
					mu = particle.landmarks[lm_id]['mean']
					q = (particle.landmarks[lm_id]['mean'][0] - particle.state[0]) **2 + (particle.landmarks[lm_id]['mean'][1] - particle.state[1]) **2
					H = np.array([ [ ((mu[0]-particle.state[0])/math.sqrt(q)).item() , ((mu[1] - particle.state[1])/math.sqrt(q)).item() ], 
						           [ (-(mu[1] - particle.state[1])/q).item()         , ((mu[0] - particle.state[0])/q).item()            ] ])


					
					# Initialise corvariance, at line 9
					H_inv = np.linalg.inv(H)
					H_inv_Transpose = H_inv.transpose()

					particle.landmarks[lm_id]['std'] = np.matmul(np.matmul(H_inv, Q_t) , H_inv_Transpose)
				...
				# Known landmark
					...
					q = (particle.landmarks[lm_id]['mean'][0] - particle.state[0]) **2 + (particle.landmarks[lm_id]['mean'][1] - particle.state[1]) **2
					z_dist_hat = np.sqrt(q)
					z_theta_hat = WrapToPi(np.arctan2( (particle.landmarks[lm_id]['mean'][1] - particle.state[1]) , (particle.landmarks[lm_id]['mean'][0] - particle.state[0]) ) - particle.state[2])
					z_hat = np.array([ z_dist_hat, z_theta_hat ])


					# Calculate Jacobian, at line 13
					mu = particle.landmarks[lm_id]['mean']
					H = np.array([ [ ((mu[0]-particle.state[0])/np.sqrt(q)).item()  , ((mu[1] - particle.state[1])/np.sqrt(q)).item() ], 
						           [ (-(mu[1] - particle.state[1])/q).item()        , ((mu[0] - particle.state[0])/q).item()             ] ])


					# Measurement covariance, at line 14
					Q = np.matmul(np.matmul(H, particle.landmarks[lm_id]['std']) , H.transpose()) + Q_t
					#print(Q.shape)


					# Calculate Kalman gain, at line 15
					Q_inv = np.linalg.inv(Q)
					K = np.matmul(np.matmul(particle.landmarks[lm_id]['std'], H.transpose()) , Q_inv)
					#print(K.shape)


					# Update mean, at line 16
					z_t = np.array([ [lm['range']], [lm['angle']] ])
					particle.landmarks[lm_id]['mean'] = particle.landmarks[lm_id]['mean'] + np.matmul(K , (z_t - z_hat))


					# Update covariance, at line 17
					particle.landmarks[lm_id]['std'] = np.matmul((np.identity(2) - np.matmul(K, H)), particle.landmarks[lm_id]['std'])
					

					# Calculate importance factor w, at line 18
					#w = 1 # Rewrite this line or update w after
					w_base = np.linalg.det(2 * pi * Q) **(-1/2)
					w_exp = np.exp((-1/2) * np.matmul(np.matmul((z_t - z_hat).transpose() , Q_inv) , (z_t - z_hat)))
					w = w_base.item() * w_exp.item()
					#print(w)
	...
	def fastslam_particles_resample(self):
		...
		total_weight = sum([particle.weight**2 for particle in self.particles])
		if (1.0 / (total_weight)) < (float(self.num_particle) / 1.5) :
			requirement = True
